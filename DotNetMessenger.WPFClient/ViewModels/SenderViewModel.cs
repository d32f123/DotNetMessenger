    ...
    public class SenderViewModel : ViewModelBase
    {
        ...
        private async Task SendMessageAsync()
        {
            await ClientApi.MessagesClient.SendMessageAsync(Chat.Id, new Message
            {
                ...
                Text = _messageText,
                Date = DateTime.Now,
                ExpirationDate = _expirationSeconds == 0 ? null : (DateTime?)DateTime.Now.AddSeconds(_expirationSeconds),
                Attachments = _attachment != null ? new List<Attachment> { _attachment } : null
            });
            MessageText = null;
            Attachment = null;
            ExpirationSeconds = 0;
